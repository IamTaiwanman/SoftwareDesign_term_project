...
$(document).ready(function () {
    ...
    var restrict = $('#img_restrict').empty();
    ...
    upload_image.append('<input type="file" name="image_file" accept=".gif,.jpg,.png,.jpeg" id="image_file">');
...
function prize_form() {
    ...
    var prize_to_group = $('#prizetogroup').val();
    console.log(open);

    var group_num = $('#addgroup div').length;
    var prize_to_group = '';
    /*for( i = 1; i <= group_num; i++){
        var tmp = group_index.concat(i);
        var group_id = $(tmp).children().eq(1).val();
        //alert(group_id)
    ...
    if (prize_title == "") {
        ...
        $.ajax({
            ...
            success: function (data) {
                //poi_mpeg(data.media);  
                $('.alert-success').show();
                $('.alert-success').append(escape('<p>更新成功!</p>'));
