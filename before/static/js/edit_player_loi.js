...
$(document).ready(function() {
    ...
    // loi_num = final_valid_or_not_Arr.length;
    // alert("loi_num = "+loi_num);
...
function initialMapMarker(){
  // alert("initialMapMarker is calling!");
  // alert(loi_num);
  items = document.querySelectorAll('.test');
            items.forEach(item => {
              $(item).prop('draggable', true)
...
function loi_form(e) {
    ...
    var open = $('#open').val();
    var valid_flag=true;
    for(var i=0;i<final_valid_or_not_Arr.length;i++){
    ...
    var route_upload_time = $('#route_upload_time').val();
...
function loi_sequence(ids) {
    if (loi_num == 0) {
        ...
        var loi_id = ids;
        var myLOI_order_Arr = document.getElementsByName('pid');
        for (var i = 0; i < myLOI_order_Arr.length; i++) {
            sequence[i] = i;
...
$('#city').change(function() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            area_select.append('<option selected disable hidden>請選擇</option>');
